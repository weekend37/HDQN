...
class HDQN(nn.Module):
    ...
    def __init__(self, env, learning_rate = 1e-3, device='cpu',
        ...
        *args, **kwars):
        ...
        # meta controller  
        self.fc_meta = nn.Sequential(nn.Linear(512, self.n_options))

        # options
        self.options = {}
        for i in range(n_options):
            self.options[i] = nn.Sequential(nn.Linear(512, self.n_outputs))
        ...
        if self.device == 'cuda':
            ...
            self.fc_hidden.cuda()
            self.fc_meta.cuda()
            [self.options[i].cuda() for i in range(n_options)]
    ...
    def get_ovals(self, state):
        ...
        fc_hidden_out = self.fc_hidden(cnn_out)
        meta_out = self.fc_meta(fc_hidden_out)
        return meta_out
    ...
    def get_qvals(self, state):
        ...
        cnn_out = self.cnn(state_t).reshape(-1, self.fc_layer_inputs)
        fc_hidden_out = self.fc_hidden(cnn_out)
        qvals = self.options[self.current_option](fc_hidden_out)
        return qvals
